...
{
	SQInteger res;
	if((sqobjtype(o1)|sqobjtype(o2)) == OT_INTEGER)
	{
...
{
	SQObjectType t1 = sqobjtype(o1), t2 = sqobjtype(o2);
	if(t1 == t2) {
		...
		assert(0);
		//if(sqobjtype(res)!=OT_INTEGER) { Raise_CompareError(o1,o2); return false; }
		//	_RET_SUCCEED(_integer(res));
...
bool SQVM::ToString(const SQObjectPtr &o,SQObjectPtr &res)
{
	...
	case OT_STRING:
		...
		return true;
	case OT_FLOAT:
		len = rsl(NUMBER_MAX_CHAR+1);
		scsprintf(_sp(len),_SC("%g"),_float(o));
		break;
	case OT_INTEGER:
		len = rsl(NUMBER_MAX_CHAR+1);
	...
	case OT_BOOL:
		len = rsl(6);
		scsprintf(_sp(len),_integer(o)?_SC("true"):_SC("false"));
		break;
	case OT_TABLE:
	case OT_USERDATA:
	case OT_INSTANCE:
	...
	default:
		len = rsl(sizeof(void*)+20);
		scsprintf(_sp(len),_SC("(%s : 0x%p)"),GetTypeName(o),(void*)_rawval(o));
	}
		...
		if(_delegable(o1)->_delegate) {
			...
			if(_delegable(o1)->GetMetaMethod(this, MT_NEXTI, closure)) {
				...
				if(CallMetaMethod(closure, MT_NEXTI, 2, itr)) {
					o4 = o2 = itr;
					if(sqobjtype(itr) == OT_NULL) _FINISH(exitpos);
...
bool SQVM::CLASS_OP(SQObjectPtr &target,SQInteger baseclass,SQInteger attributes)
{
	...
	SQObjectPtr attrs;
	if(baseclass != -1) {
		if(sqobjtype(_stack._vals[_stackbase+baseclass]) != OT_CLASS) { Raise_Error(_SC("trying to inherit from a %s"),GetTypeName(_stack._vals[_stackbase+baseclass])); return false; }
	...
	if(sqobjtype(_class(target)->_metamethods[MT_INHERITED]) != OT_NULL) {
		int nparams = 2;
		SQObjectPtr ret;
...
bool SQVM::IsEqual(const SQObjectPtr &o1,const SQObjectPtr &o2,bool &res, SQVM* vm)
{
	if(sqobjtype(o1) == sqobjtype(o2)) {
		res = (_rawval(o1) == _rawval(o2));
		
		if(!res && vm)
		{
			if(sqobjtype(o1) == OT_TABLE
				|| sqobjtype(o1) == OT_USERDATA
				|| sqobjtype(o1) == OT_INSTANCE)
			{
				if(_delegable(o1)->_delegate) {
					SQObjectPtr closure;
					SQObjectPtr eqres;
					if(_delegable(o1)->GetMetaMethod(vm, MT_EQ, closure)) {
						vm->Push(o1);
						vm->Push(o2);
						if(vm->CallMetaMethod(closure,MT_EQ,2,eqres)) {
							if(sqobjtype(eqres) != OT_BOOL) {
								vm->Raise_Error(_SC("_eq must return an boolean"));
								return false;
							}
							res = IsFalse(eqres);
						}
...
{
	if(((sqobjtype(o) & SQOBJECT_CANBEFALSE) 
		&& ( ((sqobjtype(o) == OT_FLOAT) && (_float(o) == SQFloat(0.0))) ))
#if !defined(SQUSEDOUBLE) || (defined(SQUSEDOUBLE) && defined(_SQ64))
		|| (_integer(o) == 0) )  //OT_NULL|OT_INTEGER|OT_BOOL
#else
		|| (((sqobjtype(o) != OT_FLOAT) && (_integer(o) == 0))) )  //OT_NULL|OT_INTEGER|OT_BOOL
#endif
	{
		return true;
	}
	...
	//
	{
		for(;;)
		{
			...
			case _OP_DYNAMICCAST:
				...
				{Raise_Error(_SC("cannot apply instanceof between a %s and a %s"),GetTypeName(STK(arg1)),GetTypeName(STK(arg2))); SQ_THROW();}
				//FIXME:
				//If it's casting to native types, throw an error if cast cannot be done

				//ENDFIXME
				if((sqobjtype(STK(arg2)) == OT_INSTANCE) ? (_instance(STK(arg2))->InstanceOf(_class(STK(arg1)))?true:false) : false)
				{
					TARGET = _instance(STK(arg2));
				}
				else
				{
					TARGET.Null();
...
{
	if(sqobjtype(_errorhandler) != OT_NULL) {
		SQObjectPtr out;
...
void SQVM::CallDebugHook(SQInteger type,SQInteger forcedline)
{
	...
	if(_debughook_native) {
		const SQChar *src = sqobjtype(func->_sourcename) == OT_STRING?_stringval(func->_sourcename):NULL;
		const SQChar *fname = sqobjtype(func->_name) == OT_STRING?_stringval(func->_name):NULL;
...
bool SQVM::CallNative(SQNativeClosure *nclosure, SQInteger nargs, SQInteger newbase, SQObjectPtr &retval, bool &suspend)
{
	...
	if((tcs = tc.size())) {
		for(SQInteger i = 0; i < nargs && i < tcs; i++) {
			if((tc._vals[i] != -1) && !(sqobjtype(_stack._vals[newbase+i]) & tc._vals[i])) {
				Raise_ParamTypeError(i,tc._vals[i],sqobjtype(_stack._vals[newbase+i]));
				return false;
			}
		}
	}
	...
	case OT_INSTANCE: {
		...
		if(_delegable(self)->GetMetaMethod(this, MT_GET, closure)) {
			...
			if(Call(closure, 2, _top - 2, dest, SQFalse)) {
				...
				Pop(2);
				if(sqobjtype(_lasterror) != OT_NULL) { //NULL means "clean failure" (not found)
...
{
	if(sqobjtype(key) == OT_NULL) { Raise_Error(_SC("null cannot be used as index")); return false; }
	switch(sqobjtype(self)) {
	case OT_TABLE: {
		bool rawcall = true;
		if(_table(self)->_delegate) {
