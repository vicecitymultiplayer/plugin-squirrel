...
{
	SQObjectPtr temp;
	bool belongs_to_static_table = sqobjtype(val) == OT_CLOSURE || sqobjtype(val) == OT_NATIVECLOSURE || bstatic;
	...
	if(belongs_to_static_table) {
		SQInteger mmidx;
		if((sqobjtype(val) == OT_CLOSURE || sqobjtype(val) == OT_NATIVECLOSURE) && 
			(mmidx = ss->GetMetaMethodIdxByName(key)) != -1) {
			_metamethods[mmidx] = val;
		} 
		else {
			SQObjectPtr theval = val;
			if(_base && sqobjtype(val) == OT_CLOSURE) {
				theval = _closure(val)->Clone();
				_closure(theval)->_base = _base;
				__ObjAddRef(_base); //ref for the closure
			}
			if(sqobjtype(temp) == OT_NULL) {
				bool isconstructor;
...
{
	if(sqobjtype(_class->_metamethods[mm]) != OT_NULL) {
